...
_OUTPUT_DIR  = Path(config.PDF_OUTPUT_DIR)
...
def _build_context(receipt_data: list[dict], demo: dict) -> dict:
    ...
    for r in receipt_data:
        total             = int(r.get("진료비완굴액",   0) or 0)
        copay_public      = int(r.get("공단부담완굴액", 0) or 0)
        copay_uninsured   = int(r.get("전액본인부담", 0) or 0)
        copay_uninsured_b = int(r.get("비급여완굴액",   0) or 0)
        patient_total     = int(r.get("환자부담완굴액", 0) or 0)
        # ②본인부담(급여) = 환자부담완굴액 - 전액본인부담 - 비급여을굴액
        copay_insured = int(r.get("본인부담금", 0) or 0)
        # col7 copay_cash = 환자부담완굴액(②+③+④)
